...
describe("customer profile generator", () => {
  it("summarizes questionnaire inputs and the AI conclusion into a reusable customer profile", () => {
    const profile = buildCustomerProfile(
      { brandName: "花西子", industry: "美妆个护", scale: "区域龙头", keywords: "我要出省", competition: "区域强、出省弱", goal: "拓展全国市场", background: "老板关注北上广深的试点" },
    ...
    expect(profile.title).toBe("花西子客户画像");
    expect(profile.brandName).toBe("花西子");
