...
type FormState = { brandName: string; industry: string; scale: string; keywords: string; competition: string; goal: string; background: string };
...
export default function Home() {
  ...
  const savePlaybook = () => {
    ...
    const record: StoredRecord = { id: `${Date.now()}`, title: `${form.brandName || "未命名品牌"} · ${playbook.primaryType}`, primaryType: playbook.primaryType, diagnosis: playbook.diagnosis, playbook: toReviewMarkdown(playbook, profile), createdAt: new Date().toISOString() };
    setRecords(previous => {
...
function DiagnosePanel({ form, update, submit, loading, playbook, savePlaybook, saving }: { form: FormState; update: (key: keyof FormState, value: string) => void; submit: () => void; loading: boolean; playbook: Playbook | null; savePlaybook: () => void; saving: boolean }) {
  ...
  const [multiMode, setMultiMode] = useState<Record<keyof FormState, boolean>>({ brandName: false, industry: false, scale: false, keywords: true, competition: true, goal: true, background: true });
  const questions: { key: keyof FormState; label: string; hint: string; chips: string[] }[] = [
    { key: "brandName", label: "客户品牌叫什么？", hint: "请输入品牌的正式名称，后续画像、话术和 PDF 报告都会围绕它生成", chips: [] },
  ...
  const labels: Record<keyof FormState, string> = { brandName: "品牌名称", industry: "行业", scale: "规模", keywords: "老板关键词", competition: "竞争状态", goal: "本次目标", background: "补充背景" };
